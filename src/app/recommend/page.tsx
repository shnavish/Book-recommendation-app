    ...
    <div className="min-h-screen pt-24 pb-20 px-6 container mx-auto">
      ...
      >
        ...
        <p className="text-muted-foreground text-lg flex items-center gap-2">
          ...
          Based on: <span className="text-foreground italic">&quot;{query}&quot;</span>
        </p>
      </motion.div>
          ...
          {recommendations.map((book, index) => (
            ...
            >
              <Card className="glass overflow-hidden h-full flex flex-col group border-white/5 hover:border-primary/50 transition-all duration-500 shadow-xl hover:shadow-[0_0_40px_-10px_rgba(200,169,107,0.3)] bg-gradient-to-b from-card/80 to-card/40 backdrop-blur-xl">
                <div className="relative h-56 bg-gradient-to-br from-primary/20 via-black/50 to-secondary/20 flex items-center justify-center border-b border-white/5 overflow-hidden">
                  ...
                  <motion.div 
                    ...
                    className="w-28 h-40 bg-gradient-to-br from-white/10 to-white/5 border border-white/20 rounded shadow-2xl flex items-center justify-center z-10 overflow-hidden"
                  >
                    {book.coverUrl ? (
                      <img src={book.coverUrl} alt={book.title} className="w-full h-full object-cover" />
                    ) : (
                      <BookHeart className="w-10 h-10 text-white/40 group-hover:text-primary/80 transition-all duration-500" />
                    )}
                  </motion.div>
                  <div className="absolute top-4 right-4 bg-black/80 backdrop-blur-md px-3 py-1.5 rounded-full text-xs font-bold text-primary border border-primary/30 shadow-[0_0_15px_rgba(200,169,107,0.2)]">
                ...
                <CardContent className="p-8 flex-1 flex flex-col relative">
                  ...
                  <h2 className="text-2xl font-heading font-bold mb-1 group-hover:text-primary transition-colors duration-300">{book.title}</h2>
                  <p className="text-sm text-muted-foreground mb-6 uppercase tracking-widest">by {book.author}</p>
                  
                  <div className="flex flex-wrap gap-2 mb-6">
                    {book.tags.map((tag) => (
                      <span key={tag} className="px-3 py-1 text-[10px] rounded-full bg-white/5 text-white/70 border border-white/10 uppercase tracking-wider group-hover:border-primary/30 transition-colors duration-300">
                        {tag}
                      </span>
                    ))}
                  </div>
                  ...
                  <div 
                    ...
                    className="mb-8 flex-1 cursor-pointer group/desc relative"
                  >
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-4">
                      {book.purchaseLink && (
                        <a href={book.purchaseLink} target="_blank" rel="noopener noreferrer" className="w-full">
                          <Button className="w-full bg-primary/20 hover:bg-primary/40 text-primary hover:text-white border border-primary/30 transition-all rounded-lg h-10 text-[10px] sm:text-xs font-semibold">
                            Apple Books
                          </Button>
                        </a>
                      )}
